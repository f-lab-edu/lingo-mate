...
public class WordbookService {
    ...
    // 단어장 삭제
    public Wordbook deleteWordbook(Long memberId, Long wordbookId) {
        // 1. 단어장을 조회한다.
    ...
    // 단어 생성 및 단어장에 추가
    public List<Word> createAndAddWord(Long wordbookId, List<WordCreateRequest> wordCreateRequestList) {
        // 1. 단어장 조회
    ...
    // 단어장 상세 조회
    public List<Word> getWordsInWordbook(Long wordbookId) {
        // 1. 단어장 존재 여부 확인
        Wordbook wordbook = wordbookRepository.findById(wordbookId).orElseThrow(WordbookNotFound::new);
    ...
    // 단어장에 있는 단어 수정
    public List<Word> modifyWordsInWordbook(Long wordbookId, List<WordUpdateRequest> wordUpdateRequestList) {
        // 1. 수정 대상 단어장 조회
        Wordbook wordbook = wordbookRepository.findById(wordbookId).orElseThrow(WordbookNotFound::new);
    ...
    // 단어장에 있는 단어 삭제
    public List<Word> deleteWordsInWordbook(Long wordbookId, List<WordDeleteRequest> wordDeleteRequestList) {
        // 1. 단어 삭제 대상 단어장 조회
        Wordbook wordbook = wordbookRepository.findById(wordbookId).orElseThrow(WordbookNotFound::new);
